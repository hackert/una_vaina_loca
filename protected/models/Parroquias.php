...
class Parroquias extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('idparroquia, idmunicipio', 'numerical', 'integerOnly'=>true),
			array('parroquia', 'length', 'max'=>60),
			array('codigo_ine', 'length', 'max'=>6),
			array('the_geom', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('gid, the_geom, parroquia, codigo_ine, idparroquia, idmunicipio', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'localidades' => array(self::HAS_MANY, 'Localidades', 'idparroquia'),
		);
	}
