    ...
    public function accessRules() {
        return array(
            array('allow', // allow all users to perform 'index' and 'view' actions
                'actions' => array('index', 'view', 'CargarModelo'),
                'users' => array('*'),
            ),
...
// if AJAX request (triggered by deletion via admin grid view), we should not redirect the browser
    ...
    public function actionCargarModelo() {
        $data = Modelo::model()->findAllBySql(
                "select * from track.modelo where fk_marca
		=:keyword or fk_marca=0 order by fk_marca=0 desc, descripcion asc",
                // Aquí buscamos los diferentes organismos que pertenecen al tipo elegido
                array(':keyword' => $_POST['Vehiculo']['marca']));

        if (!empty($data)) {

            $data = CHtml::listData($data, 'id_modelo', 'descripcion');
